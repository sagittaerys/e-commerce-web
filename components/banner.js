export default function Banner(){

  return(
        <div className="container banner-height">
            <div className="banner-img bg-[url('/XL.png')] bg-cover bg-center h-[420px]">

                <div className="banner-content font-semibold  banner-text max-w-[500px] h-full justify-end flex px-10 flex-col">

                    <h1 className="unique-text text-white text-[66.94px] text-bold">NEW COLLECTION</h1>
                    <h3 className="unique-text text-white text-[29.29px]">SPRING/SUMMER 2025</h3>

                </div>

            </div>
        </div>
  )
}